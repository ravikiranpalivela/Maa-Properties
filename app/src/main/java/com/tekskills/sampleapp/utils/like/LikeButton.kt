...
class LikeButton @JvmOverloads constructor(
    ...
    private var circleStartColor = 0
    private var circleEndColor = 0
    ...
    override fun onClick(v: View) {
        ...
        if (isChecked) {
            ...
            val outerCircleAnimator: ObjectAnimator = ObjectAnimator.ofFloat(
                circleView,
            ...
            val innerCircleAnimator: ObjectAnimator = ObjectAnimator.ofFloat(
                circleView,
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                icon!!.animate().scaleX(0.7f).scaleY(0.7f).setDuration(150).setInterpolator(
                    DECCELERATE_INTERPOLATOR
                )
                icon!!.animate().scaleX(1f).scaleY(1f).setInterpolator(DECCELERATE_INTERPOLATOR)
                if (isPressed) {
                    performClick()
                    isPressed = false
                }
    ...
    fun setIconSizeDp(iconSize: Int) {
        setIconSizePx(LikeUtils.dipToPixels(context, iconSize.toFloat()) as Int)
    }
    ...
    private fun parseIconType(iconType: String): Icon {
        val icons: List<Icon> = LikeUtils.icons
        for (icon in icons) {
            if (icon.iconType.name.toLowerCase()
                    .equals(iconType.lowercase(Locale.getDefault()))
            ) {
                return icon
            }
        }
    ...
    fun setExplodingDotColorsRes(@ColorRes primaryColor: Int, @ColorRes secondaryColor: Int) {
        dotsView!!.setColors(
            ContextCompat.getColor(context, primaryColor), ContextCompat.getColor(
    ...
    private fun setEffectsViewSize() {
        if (iconSize != 0) {
            dotsView!!.setSize(
                (iconSize * animationScaleFactor).toInt(),
                (iconSize * animationScaleFactor).toInt()
            )
